...
**배포됨 (2026-08-29): https://rebarsplice.vercel.app** — 폰·PC 브라우저에서 사진을
올리면 그 자리에서 검출한다. 사진은 기기 밖으로 나가지 않는다.

실데이터 학습 결과 — 모델 크기 × 입력 해상도 2축 비교 (309타일, 검증셋은
학습에 안 쓴 영상 2개 = 54타일, RTX 2060 SUPER 로컬 학습):

| 모델 | lap_splice P | R | **mAP50** | mAP50-95 | ONNX | 학습 |
|---|---|---|---|---|---|---|
| s@640 | 0.833 | 0.719 | 0.787 | 0.620 | 38MB | 23분 |
| m@640 | 0.804 | 0.780 | 0.838 | 0.675 | 82MB | 44분 |
| **l@640 ← 배포** | **0.872** | **0.818** | **0.861** | **0.700** | 99MB | 57분 |
| m@960 | 0.746 | 0.818 | 0.841 | 0.656 | 82MB | 88분 |
| l@960 | 0.868 | 0.800 | 0.857 | 0.691 | 99MB | 110분 |

읽는 법 (2026-08-29 실측):
- **모델을 키우면 (s→m→l) 전 지표가 오른다** — l 이 모든 지표 1위
- **해상도(960)는 m 의 재현율만 올렸고 l 에는 효과가 없었다** — l@960 은 l@640 에
  전부 밀린다. 두 축이 항상 합쳐지는 게 아니다
- x(230MB)는 폰 브라우저에 비현실적이라 제외했다

정확도 우선 방침(사용자 지정)이라 **l@640 을 배포**했다. 프로덕션 E2E 확인:
99MB 모델 로딩·멀티스레드·타일 추론 1.26초(데스크톱). 폰에서 3~6초 예상 —
너무 느리면 m@640 으로 교체한다(파일 복사 + 재배포 5분).

재배포:
```bash
cd web && npx vercel deploy --prod --yes
```
모델 교체는 `models/best_m.onnx` -> `web/models/best.onnx` 복사 후 위 명령.
`web/.vercelignore` 가 테스트 파일을 배포에서 뺀다.
