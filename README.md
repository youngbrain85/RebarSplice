...
실데이터 학습 결과 (309타일, 검증셋은 학습에 안 쓴 영상 2개 = 54타일):

| 모델 | lap_splice P | R | **mAP50** | 크기 | 학습 시간 |
|---|---|---|---|---|---|
| yolo26s | 0.833 | 0.719 | 0.787 | 38MB | 23분 |
| **yolo26m** ← 배포 | 0.804 | **0.780** | **0.838** | 82MB | 44분 |

정확도 우선 방침(사용자 지정)이라 **m 을 배포**했다 — 재현율 +6.1pp 가 결정적이다
(검측에서 미검출은 검사를 안 한 것). 파일럿 목표(mAP50 0.3~0.5)를 크게 넘겼다.
프로덕션에서 E2E 확인: 모델 로딩·멀티스레드(COOP/COEP)·타일 추론 950ms.
